...
export class TransactionExecutor {
  ...
  async executeArbitrage(
    ...
    try {
      ...
      // Build transaction
      const tx = this.transactionBuilder.buildArbitrageTx(route, options);
      ...
      if (options?.usePrivateRelay && this.relayManager) {
        ...
        if (relayResult.transactionHash) {
          txResponse = (await provider.getTransaction(
      ...
      const actualProfitUSD = route.expectedProfitUSD - Number(ethers.formatEther(gasCost)) * 2000; // Assume $2000 ETH
