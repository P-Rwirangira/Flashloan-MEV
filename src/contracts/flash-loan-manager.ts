...
export class FlashLoanManager extends EventEmitter {
  ...
  private readonly UNISWAP_V3_FEE_RATE = 5; // 0.05%
  ...
  async validateFlashLoanParams(params: FlashLoanParams): Promise<boolean> {
    try {
      ...
      this.emit('validationError', { params, error: error instanceof Error ? error.message : String(error) });
      return false;
    }
  }
  ...
  async calculateExpectedProfit(params: FlashLoanParams): Promise<{
    ...
    try {
      ...
      const fees = await this.calculateFlashLoanFees(
        params.flashPool,
        params.amountIn,
        0
      );
  ...
  prepareFlashLoanData(params: FlashLoanParams): {
    ...
    const routeData = abiCoder.encode(
      ...
      [
        params.route.pools,
        params.route.directions,
        params.route.minProfit,
        params.route.deadline,
      ]
    );
...
export class FlashLoanUtils {
  ...
  static calculateOptimalAmount(
    ...
    return totalReserves / 1000n; // 0.1% of total reserves
  }
   ...
   */
  static generateRouteDirections(
    pools: Address[],
    tokenIn: Address,
    tokenOut: Address
  ): boolean[] {
